...
//		add(_memoryUsage = new FlxText(10, 5, 200).setFormat(null, 8, 0xFF000000));
        ...
        float leftBtnY = 20;
        /*@formatter:off*/
        add(new FlxButton(leftBtnY, 25, "+100", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(100);
            }
        }));
        add(new FlxButton(leftBtnY, 65, "+250", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(250);
            }
        }));
        add(new FlxButton(leftBtnY, 105, "+500", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(500);
            }
        }));
        add(new FlxButton(leftBtnY, 145, "+1000", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(1000);
            }
        }));
        add(new FlxButton(leftBtnY + 90, 25, "-100", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(-100);
            }
        }));
        add(new FlxButton(leftBtnY + 90, 65, "-250", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(-250);
            }
        }));
        add(new FlxButton(leftBtnY + 90, 105, "-500", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(-500);
            }
        }));
        add(new FlxButton(leftBtnY + 90, 145, "-1000", new IFlxButton() {
            @Override
            public void callback() {
                addBunnies(-1000);
            }
