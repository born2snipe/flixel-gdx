...
public class PlayState extends FlxState {
    ...
    public void create() {
        ...
        // READ
        if (save.data.get("integer", Integer.class) != 1234) {
            FlxG.log("int failed");
        }
        if (save.data.get("float1", Float.class) != 1234567890f) {
            FlxG.log("float1 failed");
        }
        if (save.data.get("float2", Float.class) != 1234567890.123f) {
            FlxG.log("float2 failed");
        }
        if (!save.data.get("string1", String.class).equals("flixel")) {
            FlxG.log("string1 failed");
        }
        if (!save.data.get("string2", String.class).equals("flixel-gdx")) {
            FlxG.log("string2 failed");
        }
        if (!save.data.get("string3", String.class).equals("flixel-gdx rocks! Awesome!")) {
            FlxG.log("string3 failed");
        }
        if (!save.data.get("array1", Array.class).get(0).equals("hello world")) {
            FlxG.log("array1 failed");
        }

        @SuppressWarnings("unchecked")
        Array<FlxPoint> a2 = save.data.get("array2", Array.class);
        if (a2.get(0).x != 120f || a2.get(0).y != 200f) {
            FlxG.log("array2 failed");
        }

        FlxPoint p = save.data.get("object1", FlxPoint.class);
        if (p.x != 300f || p.y != 290f) {
            FlxG.log("object1 failed");
        }

        FlxPoint[] ps = save.data.get("object2", FlxPoint[].class);
        if (ps[0].x != 100f || ps[0].y != 110f) {
            FlxG.log("object2 failed");
        }
